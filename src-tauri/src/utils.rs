...
pub fn app_root() -> PathBuf {
  tauri::api::path::home_dir().unwrap().join(".catary")
}
pub fn resource_root(app: AppHandle) -> String {
  tauri::api::path::resource_dir(app.package_info(), &app.env())
    .unwrap()
    .to_string_lossy()
    .into()
}
        ...
        if update_resp.is_update_available() {
          ...
          if v {
            tauri::api::dialog::message(
              ...
              "catary",
              "Your catary is up to date",
            );
          }
        }
      }
      Err(err) => {
        tauri::api::dialog::message(app.get_window("main").as_ref(), "catary", err.to_string());
      }
    }
  });
}
